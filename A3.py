...
session.sendline('ip add 1.1.1.1 255.255.255.255')
result = session.expect(['#', pexpect.TIMEOUT, pexpect.EOF])
...
# Enables a RIP routing proccess
session.sendline('router rip')
result = session.expect(['#', pexpect.TIMEOUT, pexpect.EOF])

# Associating the appropriate address to the RIP routing process in this case being the ip address within the range of GigabitEthernet1
session.sendline('network 192.168.56.0')
result = session.expect(['#', pexpect.TIMEOUT, pexpect.EOF])

# Associating the appropriate address to the RIP routing process in this case being the ip address within the range of GigabitEthernet2
session.sendline('network 192.168.61.0')
result = session.expect(['#', pexpect.TIMEOUT, pexpect.EOF])
...
print('Conigured RIP on GigabitEthernet1 and 2')
